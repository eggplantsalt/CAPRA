# ===== CAPRA 离线挖掘主循环 (run_capra_mining.py) =====
# 作用：在 baseline 策略上运行离线挖掘，为每步生成安全监督信号。
#
# 数据流：环境reset -> K个候选动作 -> 短时CF rollout(H_s步) -> P_t/F_t
#       -> 等价集E_t -> q_hat_t -> MiningCache JSON
#
# 断点续传：已存在的 episode 缓存文件自动跳过（--force_remine 强制重跑）
#
# 快速验证：python -m experiments.robot.capra.mining.run_capra_mining \
