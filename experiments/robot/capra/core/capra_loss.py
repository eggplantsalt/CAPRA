# ===== CAPRA 训练损失计算 (capra_loss.py) =====
# 核心公式：L = L_anchor + lambda * sum_t  w_t * KL(q_hat_t || pi_theta_t)
#
# L_anchor  : 标准 L1 回归损失，每个 batch 都计算（无论是否激活 CAPRA）
# L_capra   : 在激活的时间步上求和 w_t * KL(q_hat_t || pi_theta_t)
#   q_hat_t    : 安全目标分布，来自 mining 缓存
#   pi_theta_t : 策略评分分布（候选集近似）
#   w_t        : Delta_t * (1 + rho * R_t)
#
# 连续动作空间近似：
#   pi_theta_t(a_i) ∝ exp(-gamma * ||a_i - a_hat_t||_1)  在 K 个候选上归一化
#   gamma=0 时退化为均匀先验
#
# KL 方向：KL(q_hat || pi_theta) -- 梯度流向 pi_theta（模型参数）
