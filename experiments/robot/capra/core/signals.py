# ===== CAPRA 状态信号提取器 (signals.py) =====
# 主入口：read_state_signals(obs, step, env=None, ...) -> StateSignals
#
# 信号精度：
#   物体位置/朝向   obs dict *_pos/*_quat 键  精确
#   接触冲量        sim.data.cfrc_ext         近似（力代理）
#   倾倒检测        四元数倾斜角阈值          近似
#   支撑关系        相对高度+平面距离         近似
#   工作空间违规    位置边界检查              精确
#
# 当 env/sim 不可用时（单元测试），所有读取函数优雅降级返回空值。
