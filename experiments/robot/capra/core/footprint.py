# ===== CAPRA 足迹函数 F_t(a) (footprint.py) =====
# F_t(a) = alpha_d * D_t(a)  +  alpha_i * I_t(a)  +  alpha_r * R_t(a)
#
# D_t  位移代价  = sum_{o in 惩罚对象} weight(o) * ||pos_after - pos_before||  (精确)
# I_t  接触冲量  = sum_{o in PROTECTED} cfrc_ext 力幅值  (近似)
# R_t  不可逆事件 = topple * w + support_break * w + ws_violation * w  (近似)
#
# 任务条件化：TARGET 位移不惩罚；PROTECTED 最高惩罚；NON_TARGET 低权重；IRRELEVANT 忽略
